...
int checkAssociatedBond(atom* a, int newType, atom* asso, int assoType){
  ...
  if (irx>GRID_STEP)rx-=bound[0].length;
  else if (irx<-GRID_STEP)rx+=bound[0].length;
  if (iry>GRID_STEP)ry-=bound[1].length;
  else if (iry<-GRID_STEP)ry+=bound[1].length;
  if (irz>GRID_STEP)rz-=bound[2].length;
  else if (irz<-GRID_STEP)rz+=bound[2].length;   
