...
int main(int argc, char* argv[]){
  ...
  if(naa!=p.getLength()){
    cerr << "mismatching of lengths between " 
	 << argv[1] << " " << argv[2] << endl;
    cerr << naa << " " << p.getLength() << endl;
    cerr << "The input-pdb might have multiple chains." << endl;
    cerr << "We will take the first chain." << endl;
  }
  ...
  if(naa!=first_chain_length){
    cerr << "The first chain also does not match!" << endl;
    exit(1);
  }
  ...
  int index=0;
  for(int i=0; i<first_chain_length; i++){
    ca_index[i] = index;
  ...
  for(int i=0; i<naa; i++){
    for(int j=i+2; j<naa; j++){
      if(p.getCMap()[i][j]){
	printf("%ld %ld %lf %lf %lf %lf %lf\n", cb_index[i]+1, cb_index[j]+1, minD, hcD, eREP, iD, eATTR);
      }
      else{
