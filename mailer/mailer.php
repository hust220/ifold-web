...
if ($_POST[ 'mailto'] && $_POST['subject'] && $_POST['body']){
	...
	if($_POST[ 'mailto'] == "admin") {
		$to = "\"Admin listserv\" <admin-listserv@ifold.dokhlab.org>";
	} else {
		$to = "\"User listserv\" <user-listserv@ifold.dokhlab.org>";
	...
	if($_POST[ 'mailto'] == "admin") {
		$query = "SELECT namefirst,namelast,email FROM $tableusers WHERE level='9'";
	} else if($_POST[ 'mailto'] == "all") {
		$query = "SELECT namefirst,namelast,email FROM $tableusers";
	}
	...
	$mail_headers = "From:\"iFold Server\" <ifold@dokhlab.org>\r\nReply-To:no-reply@dokhlab.org\r\nBCC:".$trimmed_bcclist."\r\nX-Mailer: iFold PHP script" . phpversion ();
