...
static double dl2[3];
static int pm1[3];
extern int ll;//defined in bcp.c
...
int allocsearch(int n)
{
 ...
 for(i=0; i<3; i++){
   dl2[i] = bound[i].dl*bound[i].dl;
   pm1[i] = bound[i].period-1;
 }
...
{ 
  ...
  i1=a[p1].i.x.i-GRID_STEP;
  i2=a[p1].i.x.i+GRID_STEP;
  j1=(a[p1].i.y.i-GRID_STEP)*search.x;
  j2=(a[p1].i.y.i+GRID_STEP)*search.x;
  if(search.z==search.y){
    k1=a[p1].i.z.i*search.y;k2=k1;
  }
  else{
    k1=(a[p1].i.z.i-GRID_STEP)*search.y;
    k2=(a[p1].i.z.i+GRID_STEP)*search.y;
  }
  ...
  for(k=k1;k<=k2;k+=search.y)
    { 
      ...
      if(addressz>=search.z)addressz-=search.z;
      for(j=j1;j<=j2;j+=search.x)
	{ 
	  addressy=j; 
	  if(addressy<0)addressy+=search.y;
	  if(addressy>=search.y)addressy-=search.y;
	  addressy+=addressz;
	  for(i=i1;i<=i2;i++)
	    { 
	      address=i; 
	      if(address<0)address+=search.x;
	      if(address>=search.x)address-=search.x;
	      address+=addressy;
		...
		if((search.collp[p2]==-1)&&(p1!=p2))
		  { 
		    search.atomp[np++]=p2;
		    if(is_en)
		      if(a[p1].rid>=0 && a[p2].rid>=0 
			 &&fabs(a[p1].rid-a[p2].rid)==1) 
			search.collp[p2]=ncoll[a[p1].c][a[p2].c];
		      else
...
{ 
 ...
 i1=a[p].i.x.i-GRID_STEP;
 i2=a[p].i.x.i+GRID_STEP;
 j1=(a[p].i.y.i-GRID_STEP)*search.x;
 j2=(a[p].i.y.i+GRID_STEP)*search.x;
 if(search.z==search.y)
   {k1=a[p].i.z.i*search.y;k2=k1;}
 else{
   k1=(a[p].i.z.i-GRID_STEP)*search.y;
   k2=(a[p].i.z.i+GRID_STEP)*search.y;
 }
...
int i0,j0,k0,address,ix,iy,iz,level;
  ...
  if(search.np=list_atoms(k0,search.atomp))
  {
    ...
    i1=a[i0].i.x.i-GRID_STEP;
    i2=a[i0].i.x.i+GRID_STEP;
    j1=(a[i0].i.y.i-GRID_STEP)*search.x;
    j2=(a[i0].i.y.i+GRID_STEP)*search.x;
    if(search.z==search.y)
      {k1=a[i0].i.z.i*search.y;k2=k1;}
    else{
      k1=(a[i0].i.z.i-GRID_STEP)*search.y;
      k2=(a[i0].i.z.i+GRID_STEP)*search.y;
    }
...
//	if((p1==299 && p2==455) || (p1==455 && p2==299)){
//	  printf("p1 %ld p2 %ld : %ld %lf @ %ld\n", p1, p2, ct, t , ll);
//	  printf("%lf %lf %lf %lf %lf %lf %ld %ld %ld\n",a[299].r.x,a[299].r.y,a[299].r.z,a[299].v.x,a[299].v.y,a[299].v.z, a[299].i.x.i, a[299].i.y.i, a[299].i.z.i);
//	  printf("%lf %lf %lf %lf %lf %lf %ld %ld %ld\n\n",a[455].r.x,a[455].r.y,a[455].r.z,a[455].v.x,a[455].v.y,a[455].v.z, a[455].i.x.i, a[455].i.y.i, a[455].i.z.i);
//	}
      }
    }
  if (q1<search.n)
...
//	  if((p2==299 && q1==455) || (p2==455 && q1==299)){
//	    printf("q1 %ld p2 %ld: %ld %lf @ %ld\n", q1, p2, ct, t , ll);
//	    printf("%lf %lf %lf %lf %lf %lf %ld %ld %ld\n",a[299].r.x,a[299].r.y,a[299].r.z,a[299].v.x,a[299].v.y,a[299].v.z,a[299].i.x.i, a[299].i.y.i, a[299].i.z.i);
//	    printf("%lf %lf %lf %lf %lf %lf %ld %ld %ld\n\n",a[455].r.x,a[455].r.y,a[455].r.z,a[455].v.x,a[455].v.y,a[455].v.z,a[455].i.x.i, a[455].i.y.i, a[455].i.z.i);
//	  }
	}
	   ...
	   if(collp[p]==-1)
	     {
	       ...
	       a1=a+p;
	       if(is_en)
		 if(a1->rid>=0 && a2->rid>=0 &&
		    fabs(a1->rid-a2->rid)==1){
		   ct=ncoll[a1->c][a2->c];
		 }
		 else{
		   ct=ecoll[a1->c][a2->c];
