...
class protein{
 ...
 public:
  ...
  int getChainInfor(int i, int& start, int&length){
    if(i<0 && i>=chains){
      return 0;
    }
    else{
      start = start_per_chain[i];
      if(i==chains-1)length = list.size()-start_per_chain[i];
      else length = start_per_chain[i+1] - start_per_chain[i];
      return 1;
    }
  }
...
protein::protein(string fileName, int isOrigin, double cutoff){
  ...
  aa* pt;
  while(1){
    tmp=line.substr(21,1);          //read the chainID
    tmp.copy(buff, 1);
    chainID = tmp[0];               
	...
	offset = num;
	if(chainID!=' '){
	  if(pre_chainID!=chainID){
	    pre_chainID = chainID;
	    pt->setChainID(chainID);
	    start_per_chain.push_back(list.size());
	    chains++;
	  }
	}
      }
      else{//BY-PASS the CAPPING
	getline(in,line);
	continue;
      }
    }
	...
	if(chainID!=pre_chainID){//a new chain
	  chains++;
	  start_per_chain.push_back(list.size());
	}
      }
	...
	if(pt->getSize()==0)               //check wheather it is the first
	  if(pt)pt->addAtom('C',x,y,z);          //creat new atom object for CA
	else{
	  errMsg = pt->getName();
	  errStatus = 3;
	  break;
	...
	if(pt && pt->getSize()==1){              //check wheather it is the second
	...
	if(ach=='C' || ach=='O' ||
	  ...
	  if(pt)pt->addAtom(ach, x, y, z);
	}
      }
    }
    
    if(!getline(in,line)){
      if(pt)list.push_back(pt);
      break;
    }
	...
	list.push_back(pt);
      ...
      if(field.substr(0,3)=="END") break;//end of a chain
      else if(field.substr(0,6)=="ENDMDL"){
	break;//end of an NMR model
      }
      else{
	int goon=0;
	while(getline(in,line)){
	  if(line.substr(0,6)==AtomField){
	    goon=1;
	    break;
	  }
