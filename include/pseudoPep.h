...
class pseudoPep{
 ...
 public:
  ...
  void constructHydrogen(){
    ...
    /*first NH*/
    {
      if(chain[0]->getID()!=PRO){
	vec ca_n = (*chain[0]->getN()->getR()) - (*chain[0]->getCA()->getR());
	ca_n.Normalize();
	vec ca_c = (*chain[0]->getC()->getR()) - (*chain[0]->getCA()->getR());
	ca_c.Normalize();
	vec n_h = ca_n*sin((H_N_CA+N_CA_C-180)*rpi)/sin(N_CA_C*rpi)
	  + ca_c*sin(H_N_CA*rpi)/sin(N_CA_C*rpi);
	n_h *= N_H;
	n_h += *chain[0]->getN()->getR();
	if(chain[0]->getH()){
	  *chain[0]->getH()->getR() = n_h;
	}
	else{
	  chain[0]->createH(n_h);
	}
  ...
  void printPDB_Backbone(ostream& out){
    int atomIndex=1;
    for(int i=0; i<chain.size(); i++)
      chain[i]->printPDB_Backbone(out, i+1, atomIndex);
  }
  ...
  int makeIndex(){
    ...
    for(int i=0; i<chain.size(); i++){
      ...
      if(chain[i]->getH())chain[i]->getH()->setIndex(index++);
