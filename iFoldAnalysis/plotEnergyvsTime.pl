...
#main function
{
  ...
  if (@ARGV<1) {
    printf("usage: plotEnergyvsTime.pl echofile\n");
    exit(1);
  }
...
#  my ($chain_id,$max_res,$min_res,$non_seq_flag,$res,$last_res);
  ...
  open(energyout,">  EnergyvsTime.dat") || die "couldn't open the energy data file!";
	
	# Number of snapshots:
  $numsnaps = @trajectory;

	for($snap_idx=1; $snap_idx<$numsnaps; $snap_idx++){
	  print energyout "$trajectory[$snap_idx]{simtime} $trajectory[$snap_idx]{energy}\n";
	}
  close(enegyout);
	system("./gnuplot $OUTPUT_UTIL_DIR/EnergyvsTime.plt");
	system("/usr/bin/convert -rotate 90 EnergyvsTime.ps EnergyvsTime.png; /bin/rm EnergyvsTime.ps");
