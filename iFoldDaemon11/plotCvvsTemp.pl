...
$OUTPUT_UTIL_DIR = "outputUtil";
  ...
  open(cvout,">  TemporaryCvvsTemp.dat") || die "couldn't open the Cv data file for writing!";
  ...
  close(cvout);
	...
	system("/bin/cat  TemporaryCvvsTemp.dat | sort -n > CvvsTemp.dat ; rm -f  TemporaryCvvsTemp.dat");
