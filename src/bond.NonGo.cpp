...
int main(){
  ...
  for(int i=0; i< npart;i++)
    for(int j=0;j<3;j++)
      partxyz[i][j]=partxyz[i][j]-cm[3]+200.0f;
  ...
  case 'y':
    ...
    double interaction[20][20];
    {    
      cout << "Input the matrix of interaction: " << endl;
      cin >> file;
      ifstream matrix_in(file);
      
      cout << "Input the sequence of the matrix: " << endl;
      cin >> file;
      FILE* matrix_seq_in;
      matrix_seq_in=fopen(file, "r");
      
      double interaction_tmp[20][20];
      for(int i=0; i<20; i++)
	for(int j=0; j<20; j++) matrix_in >> interaction_tmp[i][j];
      
      int mseqIndex[20];
      char c[10];
      stringpt=OneLetterCode;
      for(int i=0; i<20; i++){
	fscanf(matrix_seq_in,"'%1s'",c);
	for(int j=0; j<20; j++) if(strcmp(stringpt[j],c)==0)mseqIndex[i]=j;
	if(i!=19)fscanf(matrix_seq_in,",");
      }
      
      for(int i=0; i<20; i++)
	for(int j=0; j<20; j++)
	  interaction[mseqIndex[i]][mseqIndex[j]]=interaction_tmp[i][j];
    }
    ...
    for(int i=0; i<naa; i++)
      for(int j=0; j<20; j++)
	if(strcmp(seq[i],stringpt[j])==0)seqIndex[i]=j;
	...
	if(i<naa-2){
	  for(int j=i+2;j<naa;j++){
	    if(revIndex[caIndex[j]]!=0){
	      ...
	      //else ener=1.0;
	      ener=interaction[seqIndex[i]][seqIndex[j]];
	...
	ib=cbIndex[i+1];
	if(revIndex[ib]!=0)txtout << ia+1 <<" "<< ib+1 <<" "<< hcDiameter << endl;
