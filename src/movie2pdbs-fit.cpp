...
int main(int argc, char* argv[]){
  if(argc<6){
    cout << "usage: mov2pdbs.linux movie(bin) origPDB outPDB aamodel[4/2] fixProt [initFrame nFrame dN]" << endl;
  ...
  for(int j=0; j<3; j++) the_cm[j]/=(double)na;
  for(int i=0; i<na; i++)
    for(int j=0; j<3; j++){
      ref[i][j] -= the_cm[j];
    }
  ...
  new_r[0] = new double[npart*3];
  for(int i=1; i<npart; i++) new_r[i] = new_r[i-1]+3;
  ...
  while(iframe<=nframes){
	...
	r[i][2]-=(cmCurr[2]-boundary/2.0);
      ...
      for(int j=0; j<3; j++) the_cm[j]=0;
      for(int i=0; i<na; i++)
	for(int j=0; j<3; j++) {
	  snapshot[i][j] = r[ca[i]][j];
	  the_cm[j] += snapshot[i][j];
	}
      for(int j=0; j<3; j++) the_cm[j]/=(double)na;
	...
	for(int j=0; j<3; j++){
      ...
      for(int i=0; i<npart; i++){
	for(int j=0; j<3; j++){
	  new_r[i][j] = r[i][j]-the_cm[j];
	}
      }

      double rms=get_rms(snapshot, ref, na, NULL, NULL, matrix);
      //for(int i=0; i<3; i++){
	...
	for(int ip=0;ip<np;ip++){
	  for(int i=0; i<na; i++){
	    if(p.getElementAt(i)->getName()!="GLY"){
	      x=new_r[j][0]; y=new_r[j][1]; z=new_r[j][2]; j++;
	      sprintf(&field[0],"ATOM  %5d  CA  %3s %1c%4d    %8.3lf%8.3lf%8.3lf  1.00  0.00              ",
		      // j,amino[i],i+1,x,y,z);  
		      j,amino[i],chainID[i],i+1+ip*na,x,y,z);  
	      out << field << endl;
	      x=new_r[j][0]; y=new_r[j][1]; z=new_r[j][2]; j++;
	      sprintf(&field[0],"ATOM  %5d  CB  %3s %1c%4d    %8.3lf%8.3lf%8.3lf  1.00  0.00              ",
		      j,amino[i],chainID[i],i+1+ip*na,x,y,z); 
	      out << field << endl;
	    }
	    else{
