...
int main(int argc, char* argv[]){
  ...
  /*inter-chain freq contact map*/
  double** map = new double*[na];
  map[0] = new double[na*na];
  for(int i=1; i<na; i++) map[i] = map[i-1] + na;
  for(int i=0; i<na*na; i++) map[0][i] = 0;
  ...
  for(int ich=0; ich<nchain; ich++){
    p.getChainInfor(ich,istart, length);
    for(int ia=0; ia<length; ia++){
      ...
      for(int j=0; j<na; j++){
	if(map[i][j]){
	  map[i][j]/=static_cast<double>(iframe);
	  //cout << i << " " << j+1 << " " << map[i][j] << endl;
	  f_total += map[i][j];
	  i_total++;
	}
      }
      if(i_total){
	f_total /= i_total;
	cout << i+1 << " " << ich+1 << " " << istart+1 << " " << istart+length << " " << f_total << endl;
      }
    }
  }
